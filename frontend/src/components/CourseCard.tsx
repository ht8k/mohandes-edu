    ...
    >
      <div className="p-4">
        <div className="flex items-start gap-3">
          <div className="flex-1 min-w-0 text-right">
            ...
            <p className="mt-3 text-sm text-slate-600">{course.teacher.full_name}</p>
          ...
          <div className="shrink-0 flex flex-col items-center gap-2">
            <div className="w-16 h-16 rounded-2xl bg-slate-900/90 overflow-hidden flex items-center justify-center">
              {course.cover_url ? (
      ...
      <div className="border-t border-slate-100 grid grid-cols-3 divide-x divide-x-reverse divide-slate-100 text-center">
        <Stat
          ...
          value={String(course.questions_answered)}
          label="أسئلة تم الإجابة عليها"
        />
        <Stat
          icon={<Eye className="w-4 h-4 text-slate-500" />}
          value={formatCount(course.views_count)}
          label="مشاهدة"
        />
        <Stat
          icon={<Users className="w-4 h-4 text-slate-500" />}
          value={formatCount(Math.round(course.views_count / 4))}
          label="حاضناً"
        />
      </div>
    </Link>
